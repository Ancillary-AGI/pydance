...
class DeploymentManager:
    ...
    async def _build_container_image(self, source_path: str, version: str) -> str:
        ...
        image_name = f"{self.config.container_registry}/pydance:{version}"
...
# Run application
CMD ["python", "-m", "pydance.cli", "run", "--host", "0.0.0.0"]
"""
    ...
    async def _blue_green_deployment(self, image_tag: str, deployment: Deployment):
        ...
        green_service = f"pydance-green-{deployment.id}"
        ...
        await self.kubernetes_manager.switch_traffic("pydance", green_service)

        # Remove blue environment
        await self.kubernetes_manager.delete_service("pydance-blue")
    ...
    async def _canary_deployment(self, image_tag: str, deployment: Deployment):
        ...
        canary_service = f"pydance-canary-{deployment.id}"
        await self.kubernetes_manager.create_service(
            name=canary_service,
            image=image_tag,
            replicas=1,
        ...
        await self.kubernetes_manager.switch_traffic("pydance", canary_service)
    ...
    async def _rolling_deployment(self, image_tag: str, deployment: Deployment):
        ...
        await self.kubernetes_manager.update_deployment_image("pydance", image_tag)

        # Wait for rollout to complete
        await self.kubernetes_manager.wait_for_rollout("pydance")
    ...
    async def _recreate_deployment(self, image_tag: str, deployment: Deployment):
        ...
        await self.kubernetes_manager.scale_service("pydance", 0)

        # Update deployment with new image
        await self.kubernetes_manager.update_deployment_image("pydance", image_tag)

        # Scale back up
        await self.kubernetes_manager.scale_service("pydance", self.config.replicas)
