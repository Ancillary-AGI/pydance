...
def create_config_from_settings(s) -> AppConfig:
    ...
    db = DatabaseConfig(
        ...
        max_overflow=_to_int(getattr(s, 'DB_MAX_OVERFLOW', 20), 20),
        pool_timeout=_to_int(getattr(s, 'DB_POOL_TIMEOUT', 30), 30),
        echo=_to_bool(getattr(s, 'DB_ECHO', False), False),
    )

    email = EmailConfig(
        provider=EmailProvider(os.getenv('EMAIL_PROVIDER', os.getenv('MAIL_MAILER', 'smtp'))),
        host=getattr(s, 'MAIL_HOST', os.getenv('EMAIL_HOST', 'smtp.gmail.com')),
        port=_to_int(getattr(s, 'MAIL_PORT', os.getenv('EMAIL_PORT', '587')), 587),
        username=getattr(s, 'MAIL_USERNAME', os.getenv('EMAIL_USER', '')),
        password=getattr(s, 'MAIL_PASSWORD', os.getenv('EMAIL_PASSWORD', '')),
        use_tls=_to_bool(getattr(s, 'MAIL_ENCRYPTION', os.getenv('EMAIL_USE_TLS', 'True'))),
        use_ssl=_to_bool(os.getenv('EMAIL_USE_SSL', 'False')),
        from_address=getattr(s, 'MAIL_FROM_ADDRESS', os.getenv('EMAIL_FROM', 'noreply@example.com')),
        from_name=getattr(s, 'MAIL_FROM_NAME', os.getenv('EMAIL_FROM_NAME', 'App')),
    )

    server = ServerConfig(
        host=getattr(s, 'HOST', os.getenv('HOST', '127.0.0.1')),
        port=_to_int(getattr(s, 'PORT', os.getenv('PORT', '8000')), 8000),
        workers=_to_int(getattr(s, 'WORKERS', os.getenv('WORKERS', '1')), 1),
        backlog=_to_int(getattr(s, 'BACKLOG', os.getenv('BACKLOG', '2048')), 2048),
        keep_alive_timeout=_to_int(getattr(s, 'KEEP_ALIVE_TIMEOUT', os.getenv('KEEP_ALIVE_TIMEOUT', '5')), 5),
        max_connections=_to_int(getattr(s, 'MAX_CONNECTIONS', os.getenv('MAX_CONNECTIONS', '1000')), 1000),
        ssl_certfile=getattr(s, 'SSL_CERTFILE', os.getenv('SSL_CERTFILE')),
        ssl_keyfile=getattr(s, 'SSL_KEYFILE', os.getenv('SSL_KEYFILE')),
        reload=_to_bool(getattr(s, 'APP_DEBUG', getattr(s, 'RELOAD', False))),
        access_log=_to_bool(getattr(s, 'ACCESS_LOG', True)),
        debug=_to_bool(getattr(s, 'DEBUG', False)),
    )

    security = SecurityConfig(
        secret_key=getattr(s, 'SECRET_KEY', os.getenv('SECRET_KEY', 'your-secret-key-here')),
        jwt_secret=os.getenv('JWT_SECRET', ''),
        jwt_algorithm=os.getenv('JWT_ALGORITHM', 'HS256'),
        jwt_expire_minutes=_to_int(os.getenv('JWT_EXPIRE_MINUTES', '1440'), 1440),
        password_hash_rounds=_to_int(os.getenv('PASSWORD_HASH_ROUNDS', '12'), 12),
        cors_origins=getattr(s, 'ALLOWED_HOSTS', getattr(s, 'ALLOWED_HOSTS', ['localhost'])),
        allowed_hosts=getattr(s, 'ALLOWED_HOSTS', ['localhost']),
        trusted_hosts=getattr(s, 'ALLOWED_HOSTS', ['localhost']),
    )

    logging_cfg = LoggingConfig(
        level=getattr(s, 'LOG_LEVEL', os.getenv('LOG_LEVEL', 'INFO')),
        file=os.getenv('LOG_FILE', None),
        json_format=_to_bool(getattr(s, 'LOG_JSON_FORMAT', os.getenv('LOG_JSON_FORMAT', 'False'))),
        log_requests=_to_bool(getattr(s, 'LOG_REQUESTS', os.getenv('LOG_REQUESTS', 'True'))),
    )

    # Build the main AppConfig
    app_cfg = AppConfig(
        debug=_to_bool(getattr(s, 'DEBUG', False)),
        secret_key=getattr(s, 'SECRET_KEY', 'your-secret-key-here'),
        allowed_hosts=getattr(s, 'ALLOWED_HOSTS', ['localhost', '127.0.0.1']),
        database=db,
        email=email,
...
def get_config_from_environment() -> AppConfig:
    ...
    env = os.getenv('PYDANCE_ENV', 'development').lower()
...
# Default configuration - choose based on environment rather than forcing production
# This avoids raising during import when SECRET_KEY isn't set for production.
class LazyConfig:
    """Proxy for lazily-created AppConfig."""
    def __init__(self) -> None:
        self._wrapped: Optional[AppConfig] = None
    ...
    def __setattr__(self, name: str, value) -> None:  # pragma: no cover - thin proxy
        if name == '_wrapped':
...
# Lazy default config to avoid import-time side effects
default_config = LazyConfig()
