...
class PydanceClientFramework {
  constructor(config = {}) {
    ...
    this.ssrRenderer = new SSRRenderer(this);
  ...
  async _initializeCore() {
    try {
      ...
      console.log('✅ Pydance Client Framework initialized successfully');

      // Emit ready event
      this.emit('framework:ready', { framework: this });

    } catch (error) {
      console.error('❌ Failed to initialize Pydance Client Framework:', error);
      throw error;
    }
  }
  ...
  async _initializeServices() {
    ...
    if (this.config.enableAuth) {
      const auth = new Auth({
        apiClient,
        storageKey: this.config.authStorageKey || 'pydance_auth',
    ...
    if (this.config.enableTheme) {
      const theme = new ThemeManager({
        ...
        storageKey: this.config.themeStorageKey || 'pydance_theme'
      });
  ...
  emit(event, data) {
    if (this.config.debug) {
      console.log(`[PydanceClient] Event: ${event}`, data);
    }
  ...
  async mount(selector) {
    ...
    console.log('✅ Pydance Client Framework mounted successfully');
  }
  ...
  destroy() {
    ...
    console.log('✅ Pydance Client Framework destroyed');
  }
}

// Export framework instance
const framework = new PydanceClientFramework();

// Export for global usage
window.PydanceClient = framework;
...
if (typeof window !== 'undefined' && !window.PydanceClient) {
  window.PydanceClient = framework;
}
