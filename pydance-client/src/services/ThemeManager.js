...
export class ThemeManager {
  constructor(config = {}) {
    this.config = {
      ...
      storageKey: config.storageKey || 'pydance_theme',
  ...
  _applyTheme(theme) {
    ...
    Object.entries(theme.colors || {}).forEach(([key, value]) => {
      root.style.setProperty(`--pydance-${key}`, value);
    });

    Object.entries(theme.spacing || {}).forEach(([key, value]) => {
      root.style.setProperty(`--pydance-spacing-${key}`, value);
    });

    Object.entries(theme.typography || {}).forEach(([category, values]) => {
      if (typeof values === 'object') {
        Object.entries(values).forEach(([key, value]) => {
          root.style.setProperty(`--pydance-${category}-${key}`, value);
        });
      } else {
        root.style.setProperty(`--pydance-${category}`, values);
      }
    });

    Object.entries(theme.borderRadius || {}).forEach(([key, value]) => {
      root.style.setProperty(`--pydance-radius-${key}`, value);
    });

    Object.entries(theme.shadows || {}).forEach(([key, value]) => {
      root.style.setProperty(`--pydance-shadow-${key}`, value);
    });

    Object.entries(theme.transitions || {}).forEach(([key, value]) => {
      root.style.setProperty(`--pydance-transition-${key}`, value);
    });

    // Add theme class to body
    document.body.className = document.body.className.replace(/pydance-theme-\w+/g, '');
    document.body.classList.add(`pydance-theme-${themeName}`);
  ...
  emit(event, data) {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent(`pydance:theme:${event}`, { detail: data }));
    }
  }
...
export function useTheme() {
  return window.__PYDANCE_THEME__;
}
...
export function setGlobalTheme(theme) {
  globalTheme = theme;
  window.__PYDANCE_THEME__ = theme;
}
