...
export class NotificationManager {
  ...
  _initializeContainer() {
    ...
    this.container.className = `pydance-notifications pydance-notifications--${this.config.position}`;
  ...
  _initializeStyles() {
    ...
    const styleId = 'pydance-notification-styles';
    if (document.getElementById(styleId)) return;

    const style = document.createElement('style');
    style.id = styleId;
    style.textContent = `
      .pydance-notifications {
        position: fixed;
        z-index: ${this.zIndex};
        pointer-events: none;
        display: flex;
      ...
      .pydance-notifications--top-right {
        top: 20px;
        right: 20px;
      }

      .pydance-notifications--top-left {
        top: 20px;
        left: 20px;
      }

      .pydance-notifications--bottom-right {
        bottom: 20px;
        right: 20px;
        flex-direction: column-reverse;
      }

      .pydance-notifications--bottom-left {
        bottom: 20px;
        left: 20px;
        flex-direction: column-reverse;
      }

      .pydance-notifications--top-center {
        top: 20px;
        left: 50%;
        transform: translateX(-50%);
      }

      .pydance-notifications--bottom-center {
        bottom: 20px;
        left: 50%;
        transform: translateX(-50%);
        flex-direction: column-reverse;
      ...
      .pydance-notification--show {
        transform: translateX(0);
        opacity: 1;
      }

      .pydance-notification--success {
        background: linear-gradient(135deg, #10b981, #059669);
        color: white;
      }

      .pydance-notification--error {
        background: linear-gradient(135deg, #ef4444, #dc2626);
        color: white;
      }

      .pydance-notification--warning {
        background: linear-gradient(135deg, #f59e0b, #d97706);
        color: white;
      }

      .pydance-notification--info {
        background: linear-gradient(135deg, #3b82f6, #2563eb);
        color: white;
      }

      .pydance-notification__content {
        padding: 16px;
        display: flex;
      ...
      .pydance-notification__text {
        flex: 1;
        font-size: 14px;
        line-height: 1.4;
      }

      .pydance-notification__title {
        font-weight: 600;
        margin-bottom: 4px;
      }

      .pydance-notification__message {
        font-weight: 400;
        opacity: 0.9;
      }

      .pydance-notification__close {
        background: none;
        border: none;
        color: currentColor;
        cursor: pointer;
      ...
      .pydance-notification__close:hover {
        opacity: 1;
      }

      .pydance-notification__progress {
        position: absolute;
        bottom: 0;
        left: 0;
        height: 3px;
      ...
      .pydance-notification__action:hover {
        background: rgba(255, 255, 255, 0.3);
      }

      @keyframes pydance-notification-shake {
        0%, 100% { transform: translateX(0); }
        25% { transform: translateX(-5px); }
        75% { transform: translateX(5px); }
      }

      .pydance-notification--shake {
        animation: pydance-notification-shake 0.5s ease-in-out;
      }
    `;
  ...
  _renderNotification(notification) {
    ...
    element.className = `pydance-notification pydance-notification--${notification.type}`;
    ...
    element.innerHTML = `
      <div class="pydance-notification__content">
        ${icon ? `<div class="pydance-notification__icon">${icon}</div>` : ''}
        <div class="pydance-notification__text">
          <div class="pydance-notification__title">${this._escapeHtml(notification.title)}</div>
          <div class="pydance-notification__message">${this._escapeHtml(notification.message)}</div>
        </div>
        <button class="pydance-notification__close" aria-label="Close notification">×</button>
      </div>
      ${notification.actions.length > 0 ? `
        <div class="pydance-notification__actions">
          ${notification.actions.map(action => `
            <button class="pydance-notification__action" data-action="${action.key}">
      ...
      ${this.config.showProgress && notification.duration > 0 ? `
        <div class="pydance-notification__progress" style="width: 100%"></div>
      ` : ''}
    `;

    // Add event listeners
    const closeBtn = element.querySelector('.pydance-notification__close');
    closeBtn.addEventListener('click', () => this.remove(notification.id));

    // Add action listeners
    const actionBtns = element.querySelectorAll('.pydance-notification__action');
    ...
    requestAnimationFrame(() => {
      element.classList.add('pydance-notification--show');
    });
  }
  ...
  _updateProgress(id) {
    ...
    const element = this.container?.querySelector(`[data-notification-id="${id}"] .pydance-notification__progress`);
    if (element) {
  ...
  remove(id) {
    ...
    if (element) {
      element.classList.remove('pydance-notification--show');
      setTimeout(() => {
  ...
  _updateNotificationElement(id) {
    ...
    // Update content
    const titleEl = element.querySelector('.pydance-notification__title');
    const messageEl = element.querySelector('.pydance-notification__message');

    if (titleEl) titleEl.textContent = notification.title;
    if (messageEl) messageEl.textContent = notification.message;

    // Update type class
    element.className = element.className.replace(/pydance-notification--\w+/, `pydance-notification--${notification.type}`);
  }
  ...
  emit(event, data) {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent(`pydance:notification:${event}`, { detail: data }));
    }
  }

  on(event, handler) {
    if (typeof window !== 'undefined') {
      window.addEventListener(`pydance:notification:${event}`, (e) => handler(e.detail));
    }
  }
  ...
  destroy() {
    ...
    // Remove styles
    const style = document.getElementById('pydance-notification-styles');
    if (style) {
      style.remove();
    }
  }
...
export function useNotifications() {
  return window.__PYDANCE_NOTIFICATIONS__;
}
...
export function setGlobalNotifications(notifications) {
  ...
  window.__PYDANCE_NOTIFICATIONS__ = notifications;
}
