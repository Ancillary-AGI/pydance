...
export class CacheManager {
  ...
  async _initializeIndexedDB() {
    ...
    try {
      const request = indexedDB.open('PydanceCache', 1);
  ...
  _loadPersistedCaches() {
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key?.startsWith('pydance_cache_')) {
  ...
  _clearLocalStorage() {
    const keys = Object.keys(localStorage).filter(key => key.startsWith('pydance_cache_'));
  ...
  _clearSessionStorage() {
    const keys = Object.keys(sessionStorage).filter(key => key.startsWith('pydance_cache_'));
...
export function useCache() {
  return window.__PYDANCE_CACHE__;
}
...
export function setGlobalCache(cache) {
  globalCache = cache;
  window.__PYDANCE_CACHE__ = cache;
}
