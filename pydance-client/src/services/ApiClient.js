/**
 * ApiClient - Enhanced HTTP Client for Pydance Integration
 * Provides robust HTTP client with interceptors, caching, retry logic,
 * and seamless integration with Pydance backend
 */
...
export class ApiClient {
  ...
  async _authInterceptor(url, config) {
    const token = localStorage.getItem('pydance_auth_token');
    if (token) {
      config.headers = {
        ...config.headers,
  ...
  setAuthToken(token) {
    ...
    localStorage.setItem('pydance_auth_token', token);
  }

  removeAuthToken() {
    delete this.config.headers['Authorization'];
    localStorage.removeItem('pydance_auth_token');
  }
